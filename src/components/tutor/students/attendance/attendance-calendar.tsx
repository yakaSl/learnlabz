        ...
        <CardHeader>
             <div className="flex items-center justify-between flex-wrap gap-2">
                <div>
                    ...
                    <CardDescription>Click a date to edit the record.</CardDescription>
                </div>
                <div className="flex items-center gap-2">
                    <Button variant="outline" size="icon" onClick={prevMonth} className="h-8 w-8">
                        <ChevronLeft className="h-4 w-4" />
                    </Button>
                    <h2 className="text-lg font-semibold hidden sm:block">{format(currentDate, 'MMMM yyyy')}</h2>
                    ...
                    <TooltipProvider key={day.toString()}>
                        <Tooltip>
                             <TooltipTrigger asChild>
                                <div
                                className={cn(
                                    "relative p-1 border-b border-r flex flex-col items-center justify-center h-20 group cursor-pointer",
                            ...
                            {dayAttendance && (
                                <TooltipContent>
                                    <p>{dayAttendance.status} in {dayAttendance.class} on {format(day, 'MMM d')}</p>
                                </TooltipContent>
                            )}
                        </Tooltip>
                    </TooltipProvider>
